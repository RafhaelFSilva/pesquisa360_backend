...
def create_coleta(
    ...
    """Cria uma nova coleta, valida geofence e salva respostas."""
    ...
    # --- 1. Converte ponto de início para WKT ---
    ...
    if pesquisa.cerca_eletronica and localizacao_inicio_wkt:

        # Converte WKB → WKT usando CAST (forma correta!)
        cerca_wkt = db.scalar(
            text("SELECT ST_AsText(CAST(:geom AS geometry))")
            .bindparams(bindparam("geom", pesquisa.cerca_eletronica.data))
        )

        # Garante prefixo com SRID
        cerca_wkt = f"SRID=4326;{cerca_wkt}"

        # Usa ST_DWithin com Geography (aceita polígono e ponto)
    ...
    # --- 3. Converte ponto final para WKT ---
    localizacao_fim_wkt = (
